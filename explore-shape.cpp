...
#include <string>
 ...
 // Add required OpenCASCADE headers
...
#include <GeomAbs_SurfaceType.hxx> // For GeomAbs_Plane and other constants
#include <TopAbs_Orientation.hxx> // For TopAbs_FORWARD and other constants
#include <BRepAdaptor_Surface.hxx> // For BRepAdaptor_Surface
...
std::string SurfaceTypeName(GeomAbs_SurfaceType t)
{
    switch (t)
    {
    case GeomAbs_Plane:
...
std::string OrientationName(TopAbs_Orientation o)
{
    switch (o)
    {
    case TopAbs_FORWARD:
...
void get_vertex_points_test1(const TopoDS_Shape& shape)
{
    ...
    // Update to use Handle(TColgp_HArray1OfPnt) directly instead of Handle_TColgp_HArray1OfPnt
